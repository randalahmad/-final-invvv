...
export function ExtraPreviewScreen({ path }: { path: string }) {
  if (path.startsWith("/impact/")) return <div className="space-y-5"><Link href="/impact" className="text-xs text-muted">العودة إلى سجل الأثر</Link><PageHeader title="ملف أثر: المساعد الذكي للخدمات" description="نموذج معاينة يوضح ارتباط القياسات بالحل المسجل دون حفظ."/><div className="grid gap-4 md:grid-cols-3">{[["خفض زمن الخدمة","تشغيلي","45 دقيقة","20 دقيقة","24 دقيقة"],["خفض تكلفة المعاملة","مالي","38 ريال","20 ريال","23 ريال"],["رضا المستفيدين","المستفيدون","76%","90%","88%"]].map(([name,type,base,target,actual])=><Card key={name}><CardHeader><Badge variant="primary">{type}</Badge><CardTitle>{name}</CardTitle></CardHeader><CardContent className="space-y-2 text-xs"><p>خط الأساس: <b>{base}</b></p><p>المستهدف: <b>{target}</b></p><p>الفعلي: <b>{actual}</b></p><p>فترة القياس: الربع الثاني 2026</p><p>المصدر: منصة الخدمات الرقمية</p><Badge variant="success">تم التحقق داخلياً</Badge></CardContent></Card>)}</div><p className="text-xs text-muted">هذه قياسات تشغيلية تجريبية ولا تمثل اعتماداً رسمياً من الهيئة.</p></div>;
