...
export async function loadRequirementWorkspace(actor: AccessContext, requirementId: string) {
  ...
  const assignment = await prisma.complianceRequirementAssignment.findFirst({
    ...
    include:{ requirement:true, department:{include:{organization:true}} }, orderBy:{createdAt:"asc"}
  });
  ...
  return { assignment, evidence:links.map(({evidence})=>({id:evidence.id,title:evidence.title,classification:evidence.classification,fileName:evidence.fileName,version:evidence.version,reviewStatus:evidence.reviewStatus,uploadedAt:evidence.createdAt.toISOString(),uploader:evidence.uploadedBy?.name??"—"})), counts, canEdit:canMutate(actor,config.code,assignment.departmentId) };
}
...
export async function saveRequirementWorkspace(actor: AccessContext, requirementId:string, data:WorkspaceData) {
  ...
  const status=deriveOperationalStatus(config,data,loaded.counts);
...
export async function uploadRequirementEvidence(actor:AccessContext,requirementId:string,evidenceType:string,file:EvidenceFileInput) {
  ...
  try { return await prisma.$transaction(async(tx)=>{ const previous=await tx.evidence.findFirst({where:{links:{some:{entityType:"REQUIREMENT_ASSIGNMENT",entityId:loaded.assignment.id}},classification:evidenceType,archivedAt:null},orderBy:{version:"desc"},select:{version:true}}); const created=await tx.evidence.create({data:{title:config.evidence.find((r)=>r.key===evidenceType)!.title,classification:evidenceType,fileName:validated.fileName,mimeType:validated.mimeType,sizeBytes:validated.sizeBytes,checksum:validated.checksum,storagePath:key,version:(previous?.version??0)+1,uploadedById:actor.userId,reviewStatus:"DRAFT",fileProcessingStatus:"UPLOADED"}}); await tx.evidenceLink.create({data:{evidenceId:created.id,entityType:"REQUIREMENT_ASSIGNMENT",entityId:loaded.assignment.id,requirementId:loaded.assignment.complianceRequirementId}}); const counts={...loaded.counts,[evidenceType]:(loaded.counts[evidenceType]??0)+1}; const status=deriveOperationalStatus(config,loaded.assignment.workspaceData as WorkspaceData,counts); await tx.complianceRequirementAssignment.update({where:{id:loaded.assignment.id},data:{operationalStatus:status,lastSavedById:actor.userId}}); await writeAudit({actorUserId:actor.userId,action:AUDIT.EVIDENCE_UPLOADED,entityType:"EVIDENCE",entityId:created.id,departmentId:loaded.assignment.departmentId,summary:"رفع إثبات لمتطلب تشغيلي",metadata:{requirementCode:config.code,evidenceType,assignmentId:loaded.assignment.id,version:created.version}},tx); return {id:created.id,status}; }); } catch(error){ try{await storage.delete(key);}catch{} throw error; }
}
