      ...
      {visibleUnits.length ? <section>
        ...
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {visibleUnits.map((unit) => {
            ...
            return <Link key={unit.code} href={href(unit.href, persona)} className="group"><Card className="h-full transition hover:border-primary/40 hover:shadow-md"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><Badge variant="primary">وحدة عمل</Badge><CardTitle className="mt-2 text-base">{unit.name}</CardTitle></div><span className="text-2xl font-bold text-primary">{unit.readiness}%</span></div></CardHeader><CardContent className="space-y-4"><Progress value={unit.readiness} /><div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg bg-slate-50 p-2 dark:bg-white/5"><strong className="block text-sm">{completed}/{unit.requirements.length || "—"}</strong>مكتملة</div><div className="rounded-lg bg-slate-50 p-2 dark:bg-white/5"><strong className="block text-sm text-warning">{unit.missingEvidence}</strong>أدلة ناقصة</div><div className="rounded-lg bg-slate-50 p-2 dark:bg-white/5"><strong className="block text-sm text-danger">{unit.actionRequired}</strong>تحتاج إجراء</div></div><div className="flex items-center justify-between border-t pt-3 text-xs text-muted"><span>آخر تحديث: {unit.lastUpdate}</span><ArrowLeft className="h-4 w-4 transition group-hover:-translate-x-1" /></div></CardContent></Card></Link>;
          })}
        </div>
