...
export async function RequirementRoute({unitIndex,requirementId}:{unitIndex:number;requirementId:string}){
  ...
  try {
    ...
    const contributions=requirementId==="5-23-1-r1"?await listContributionsForAssignment(loaded.assignment.id):[];
    ...
    return <OperationalWorkspace unit={unit} requirement={requirement} config={config} initial={loaded.assignment.workspaceData as WorkspaceData} initialEvidence={loaded.evidence} initialStatus={loaded.assignment.operationalStatus as OperationalStatus} canEdit={loaded.canEdit} governance={governance} contributions={contributions}/>;
