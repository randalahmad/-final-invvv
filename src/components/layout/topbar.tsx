...
export function Topbar({ userName, userEmail = "", preview = false, innovatorPreview = false, canViewCompliance = false, alerts = [], developmentRoleOptions = [] }: { userName: string; userEmail?: string; preview?: boolean; innovatorPreview?: boolean; canViewCompliance?: boolean; alerts?: AlertItemData[]; developmentRoleOptions?: DevelopmentRolePreviewOption[] }) {
  const searchParams = useSearchParams();
  const persona = previewPersonaFromSearch(searchParams.get("previewRole"));
  const showReports = !innovatorPreview && (preview ? permissionsForPreviewPersona(persona).includes("compliance.view") : canViewCompliance);
