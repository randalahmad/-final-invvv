...
export default async function MyTasksPage(){
  ...
  return <div className="flex flex-col gap-5"><PageHeader title="مهامي" description="العمل القابل للتنفيذ المسند إليك، منفصل عن التنبيهات العامة."/><Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-xs"><thead><tr className="border-b text-muted">{["المهمة","المعيار والمتطلب","الإدارة","طالب الإجراء","الأولوية","الموعد","الحالة","الإجراء التالي"].map(x=><th key={x} className="p-3 text-start">{x}</th>)}</tr></thead><tbody>{tasks.map(task=>{
    ...
    const governanceCorrective=task.sourceKey?.startsWith("governance-corrective:")?task.sourceKey.split(":"):null;
    const governanceTask=task.sourceKey?.startsWith("governance-task:")?task.sourceKey.split(":"):null;
    const cultureActivity=task.sourceKey?.startsWith("culture-activity:")?task.sourceKey.split(":"):null;
    const mechanismStage=task.sourceKey?.startsWith("mechanism-stage:")?task.sourceKey.split(":"):null;
    const href=cooperationActivation?`/activities/requirements/5-23-2-r4/activations/${cooperationActivation[1]}#activation-${cooperationActivation[2]}`:openInnovation?`/activities/requirements/5-23-2-r3/events/${openInnovation[1]}#event-${openInnovation[2]}`:governanceCorrective?`${base}#corrective-${governanceCorrective[2]}`:governanceTask?`${base}#task-${governanceTask[2]}`:cultureActivity?`/governance/requirements/5-23-3-r3/activities/${cultureActivity[1]}#task-${cultureActivity[2]}`:mechanismStage?`/governance/requirements/5-23-3-r4/mechanism/${mechanismStage[1]}#task-${mechanismStage[3]}`:task.methodologyApplicationId?`${base}?methodologyCaseId=${task.methodologyApplicationId}#methodology-task-context`:task.activityId?`${base}?activityId=${task.activityId}#activity-tasks`:task.committeeId?`/governance/committees/${task.committeeId}#task-${task.id}`:base;
    return <tr key={task.id} className="border-b"><td className="p-3"><Badge variant="primary">{typeLabels[task.type]}</Badge><p className="mt-1 font-semibold">{task.title}</p>{task.activity?<p className="text-muted">النشاط: {task.activity.nameAr}</p>:task.methodologyApplication?<p className="text-muted">حالة المنهجية: {task.methodologyApplication.nameAr}</p>:task.committee?<p className="text-muted">الوحدة/اللجنة: {task.committee.nameAr}</p>:openInnovation?<p className="text-muted">فعالية ابتكار مفتوح</p>:cultureActivity?<p className="text-muted">نشاط نشر ثقافة الابتكار</p>:mechanismStage?<p className="text-muted">مرحلة آلية إدارة الابتكار الرقمي</p>:null}</td><td className="p-3">{task.assignment.requirement.code}<br/>{task.assignment.requirement.titleAr}</td><td className="p-3">{task.assignment.department.nameAr}</td><td className="p-3">{task.requestedById}</td><td className="p-3">{priorityLabels[task.priority]}</td><td className="p-3">{task.dueDate?.toLocaleDateString("ar-SA")??"—"}</td><td className="p-3">{statusLabels[task.status]??task.status}</td><td className="p-3"><Link className="text-primary" href={href}>{task.nextAction??"فتح المتطلب"}<ArrowLeft className="inline h-4 w-4"/></Link></td></tr>;
