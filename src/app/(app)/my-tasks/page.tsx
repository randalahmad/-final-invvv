...
export default async function MyTasksPage(){
  ...
  return <div className="flex flex-col gap-5"><PageHeader title="مهامي" description="العمل القابل للتنفيذ المسند إليك، منفصل عن التنبيهات العامة."/><Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-xs"><thead><tr className="border-b text-muted">{["المهمة","المعيار والمتطلب","الإدارة","طالب الإجراء","الأولوية","الموعد","الحالة","الإجراء التالي"].map(x=><th key={x} className="p-3 text-start">{x}</th>)}</tr></thead><tbody>{tasks.map(task=>{
    ...
    const href=cooperationActivation?`/activities/requirements/5-23-2-r4/activations/${cooperationActivation[1]}#activation-${cooperationActivation[2]}`:openInnovation?`/activities/requirements/5-23-2-r3/events/${openInnovation[1]}#event-${openInnovation[2]}`:governanceCorrective?`${base}#corrective-${governanceCorrective[2]}`:governanceTask?`${base}#task-${governanceTask[2]}`:task.methodologyApplicationId?`${base}?methodologyCaseId=${task.methodologyApplicationId}#methodology-task-context`:task.activityId?`${base}?activityId=${task.activityId}#activity-tasks`:task.committeeId?`/governance/committees/${task.committeeId}#task-${task.id}`:base;
