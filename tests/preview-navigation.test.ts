...
describe("DGA permission-aware navigation",()=>{
  it("keeps the five DGA units primary while exposing authorized supporting routes",()=>{const hrefs=navGroups.flatMap(group=>group.items.map(item=>item.href));for(const href of ["/strategy","/activities","/governance","/solutions","/impact","/challenges","/partners"])expect(hrefs).toContain(href);});
