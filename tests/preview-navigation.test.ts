...
describe("preview URL authority",()=>{
  ...
  it("exposes challenges and partnerships only to the relevant preview personas",()=>{expect(PREVIEW_PERSONA_PATHS.internal).toEqual(expect.arrayContaining(["/challenges","/partners"]));expect(PREVIEW_PERSONA_PATHS.partner).toContain("/partners");expect(PREVIEW_PERSONA_PATHS.innovator).not.toContain("/challenges");});
});
